...
template alignMask*(value, mask: untyped): untyped =
  (((value) + (mask)) and ((not 0) and not(mask)))

template `+`*[T](p: ptr T, off: Natural): ptr T =
  cast[ptr type(p[])](cast[int](p) +% int(off) * sizeof(p[]))
...
template `-`*[T](p1, p2: ptr T): ptr T =
  cast[ptr type(p1[])]((cast[int](p1) -% cast[int](p2)) / sizeof(p1[]))
...
proc poolPageCreate(pool: ptr Pool): ptr Page =
  ...
  result.next = nil
  for i in 0 ..< cap:
    result.ptrs[cap - i - 1] = result.buff + c_sizet(i) * c_sizet(itemSize)
...
proc poolCreate*(itemSize, capacity: int): ptr Pool =
  ...
  page.next = nil
  for i in 0 ..< cap:
    page.ptrs[cap - i - 1] = page.buff + c_sizet(i) * c_sizet(itemSize)
